<?php
/**
 * Script directo para crear usuarios
...
echo "<h2>🔧 Creando usuarios directamente...</h2>";
...
try {
    ...
    echo "<p>🗑️ Usuario administrador anterior eliminado (si existía)</p>";
    ...
    echo "<p>🗑️ Usuario empleado anterior eliminado (si existía)</p>";
    ...
    // ========================================
    echo "<h3>🔍 Verificando usuarios creados...</h3>";
    
    $stmt = $pdo->prepare("SELECT id, nombre, email, rol FROM usuarios WHERE email IN (?, ?)");
    $stmt->execute([$adminEmail, $empleadoEmail]);
